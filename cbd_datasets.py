...
class GEODEDataset(datasets.ImageFolder):
    ...
    def __init__(self,
        ...
        super(GEODEDataset, self).__init__(
            root=img_root,
            transform=None,
            target_transform=None,
            loader=loader,
        ...
        self.redefine_class_to_idx(meta_data)
        ...
        #         if bool(int(use_train)) == self.train:
        #             indices_to_use.append(int(idx))
        indices_to_use = [i for i in range(len(meta_data))]  # use all indices for now
        ...
        #         if int(idx) in indices_to_use:
        #             filenames_to_use.add(fn)
        # filenames_to_use = set(list(meta_data['file_path']))

        # TODO: what does this do? filename : image idx
        # img_paths_cut = {img_path: idx for idx, (img_path, lb) in enumerate(self.imgs)}
        # imgs_to_use = [self.imgs[img_paths_cut[os.path.join(img_root, fn)]] for fn in filenames_to_use]
        imgs_to_use = self.imgs
    ...
    def __getitem__(self, index):
        # generate one sample
        sample, target = super(GEODEDataset, self).__getitem__(index)
    ...
    def redefine_class_to_idx(self, data):
        adjusted_dict = {}
        class_list = list(data['object'].unique())
        for idx, cls in enumerate(class_list):
            adjusted_dict[cls] = idx
        self.class_to_idx = adjusted_dict
        # for k, v in self.class_to_idx.items():
        #     k = k.split('.')[-1].replace('_', ' ')
        #     split_key = k.split(' ')
